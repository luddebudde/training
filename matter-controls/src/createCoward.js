...
export const createCoward = (players, getGameObjects, position,  homePos) => {
  ...
  return {
    ...
    update: () => {
      ...
      const cowardBodies = getGameObjects().filter((obj) => obj.type === 'coward').map(obj => obj.body).filter(body => body)
      ...
      const isClose = isDistanceLessThan(player.body.position, body.position, 500)
