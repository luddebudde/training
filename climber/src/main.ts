...
import { down, front, left, origo, up } from './vectors.ts'
import {
  springForce,
  thrustBackward,
  thrustForward,
...
const sunLight: DirectionalLight = new DirectionalLight(0xffffff, 0.9)
...
const initThree = (): Game => {
  ...
  sunLight.shadow.camera.far = lightDistance * 2
...
const render = (game: Game) => {
  ...
  skyScene.position.copy(vector3(player1.body.position))
  renderer1.render(scene, camera1)

  setSunlight(player2, sunLight, lightDistance)
  skyScene.position.copy(vector3(player2.body.position))
...
const updatePhysics = (timeSinceLastCalled: number, game: Game) => {
  ...
  players.forEach((player) => {
    ...
    applyTorqueToAlign(player.body, alignNorm)

    // Camera calculation
    const forwardDir = player.body.vectorToWorldFrame(front)
    const directionOnSurface = projectOnSurface(forwardDir, alignNorm)
    ...
    const cameraPos = player.body.position
      .vadd(alignNorm.scale(cameraDistanceAbove))
    ...
    player.camera.up.copy(vector3(alignNorm))
  ...
  gameObjectIndex.all.forEach((obj) => {
    ...
    if (obj.gravitational) {
      const surfaceNorm = down
      const hoverHeight = 5
      const surfacePos = projectOnSurface(obj.body.position, surfaceNorm).vadd(
        up.scale(hoverHeight),
      )
...
const setSunlight = (
  ...
  sunLight.position.copy(
    vector3(player.body.position.vadd(up.scale(lightDistance))),
  )
  sunLight.quaternion.setFromAxisAngle(vector3(up), 0)
...
const createCannonBall = (radius: number): GameObject => {
  const mass = 1  
...
const createPlayer = (position: Vec3, camera: OrthographicCamera): Player => {
  ...
  // The nose
  const cannonPos = shapeBody.halfExtents.vadd(bodyOffset)
  const body = new Body({
    mass: 1,
    linearDamping: 0.9,
    angularDamping: 0.95,
  ...
  return {
    ...
    shoot: throttle(
      ...
      () => {
        const ballRadius = 1
        const forward = body.vectorToWorldFrame(front)
        forward.normalize()
        const impulse = forward.scale(5)

        const spawnPosition = body.pointToWorldFrame(
          cannonPos.vadd(front.scale(ballRadius)),
        )

        const cannonBall = createCannonBall(ballRadius)
        cannonBall.body.position.copy(spawnPosition)
        cannonBall.body.applyImpulse(impulse, cannonBall.body.position)
        addGameObject(cannonBall)
        body.applyImpulse(impulse.negate(), body.position)
      },
      {
        noTrailing: true,
      },
...
const createBox = (): GameObject => {
  const length = 10
  const halfLength = length / 2
  const shape = new Box(new Vec3(halfLength, halfLength, halfLength))
  const body = new Body({
    position: up.scale(length),
    shape,
    mass:
      shape.halfExtents.x *
      2 *
...
const createGround = (): GameObject => {
  ...
  body.quaternion.setFromAxisAngle(left, Math.PI / 2)
